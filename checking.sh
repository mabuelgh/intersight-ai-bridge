...
if sudo docker run hello-world; then
    echo "Docker is installed and is running successfully."
else
    echo "Failed to run Docker hello-world container."
    exit 1
fi
...
if sudo nvidia-smi; then
    echo "Nvidia drivers installation is successfull."
else
...
if sudo docker run --rm --runtime=nvidia --gpus all ubuntu nvidia-smi; then
    echo "Nvidia container tookit is installed and is running successfully."
elif [[ $? -ne 0 ]]; then
    echo "Failed to install Nvidia container tookit : an error occurred."
    ...
    if [[ $output == *"Failed to initialize NVML: Unknown Error"* ]]; then
        ...
        if sudo sudo docker run --rm --runtime=nvidia --gpus all ubuntu nvidia-smi; then
            echo "Nvidia container tookit is now running successfully!"
        else
            echo "Failed to install Nvidia container tookit : an unexpected error occurred."
            exit 1
        fi
    else
        echo "Failed to install Nvidia container tookit : an unexpected error occurred."
